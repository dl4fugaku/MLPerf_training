...
def get_models():
    ...
    """
    all_models = io.gfile.glob(os.path.join(MODELS_DIR, '*.meta'))
...
def game_counts(n_back=20):
    ...
    for m in model_filenames[:n_back]:
        games = io.gfile.glob(os.path.join(SELFPLAY_DIR, m, '*.zz'))
        print(m, len(games))
...
def rl_loop():
    ...
    def count_games():
      ...
      if not os.path.exists(os.path.join(SELFPLAY_DIR, model_name)):
        ...
        return 0
      return len(io.gfile.glob(os.path.join(SELFPLAY_DIR, model_name, '*.zz')))
