...
def train(
    ...
    tf_records = sorted(io.gfile.glob(os.path.join(chunk_dir, '*.tfrecord.zz')))
...
def validate(
    ...
    with timer("Building lists of holdout files"):
        for record_dir in tf_record_dirs:
            tf_records.extend(io.gfile.glob(os.path.join(record_dir, '*.zz')))
...
def selfplay(
    ...
    with io.gfile.GFile(os.path.join(clean_sgf, '{}.sgf'.format(output_name)), 'w') as f:
        f.write(player.to_sgf(use_comments=False))
    with io.gfile.GFile(os.path.join(full_sgf, '{}.sgf'.format(output_name)), 'w') as f:
...
def gather(
    ...
    models = [model_dir.strip('/')
              for model_dir in sorted(tf.gfile.ListDirectory(input_directory))[-50:]]
    ...
    try:
        with io.gfile.GFile(meta_file, 'r') as f:
    ...
    with io.gfile.GFile(meta_file, 'w') as f:
